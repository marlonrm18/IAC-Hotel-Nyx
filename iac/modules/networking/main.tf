...
# single_nat_gateway=true → 1 EIP (dev/test, ahorro de costo)
# single_nat_gateway=false → 1 EIP por AZ (producción, alta disponibilidad)
...
# ─── Route table pública (compartida por las 2 AZs) ──────────────────────────
...
# Con single_nat_gateway=true ambas apuntan al mismo NAT GW (índice 0).
# Con single_nat_gateway=false cada AZ usa su propio NAT GW → sin SPOF.
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name = "${var.project}-${var.environment}-rt-private-${var.availability_zones[count.index]}"
  }
}
...
# Solucion al fallo de checkov "CKV2_AWS_12"
